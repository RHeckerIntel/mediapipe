...
TEST(ClassificationCalculatorTest, TestImageClassification) {
  ...
  auto &classification = output_packets[0].Get<geti::InferenceResult>();
  ASSERT_EQ(classification.rectangles.size(), 1);

  ASSERT_EQ(classification.rectangles[0].shape.height, raw_image.rows);
  ASSERT_EQ(classification.rectangles[0].shape.width, raw_image.cols);

  const auto &cow_map = classification.saliency_maps[0];
  ASSERT_EQ(cow_map.label.label_id, "653bb9844e88964031d81e30");

  const auto &sheep_map = classification.saliency_maps[1];
