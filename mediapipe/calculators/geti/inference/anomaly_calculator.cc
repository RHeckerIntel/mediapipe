...
#endif
  cc->Outputs().Tag("INFERENCE_RESULT").Set<geti::InferenceResult>();
...
absl::Status AnomalyCalculator::GetiProcess(CalculatorContext *cc) {
  ...
  LOG(INFO) << "AnomalyCalculator before infer";
  auto infer_result = model->infer(cvimage);
  LOG(INFO) << "AnomalyCalculator after infer";
